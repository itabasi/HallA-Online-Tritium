...
double ana::CoinCalc_gogami_c(int RS2_seg, int LS2_seg,int rhit, int lhit){
  ...
  // Get Tdc Offset //
  // CID_F1S2 ==1; RHRS : lr ==1, LHRS : lr ==0;
  // TOP-PMT tb ==0, BOT-PMT tb==1
  // 

  // TOP-PMT == Left PMT , BOT-PMT == Right PMT
  double tref_R  = RTDC_F1FirstHit[9]          * tdc_time; // RHRS  Ref
  double tref_L  = LTDC_F1FirstHit[40]         * tdc_time; // LHRS  Ref

  //  cout<<"RS2_seg : "<<RS2_seg<<" off "<<RS2_t_off[RS2_seg]<<" time "<<<<endl;

  double time_Rt = (RTDC_F1FirstHit[RS2_seg+16] - RS2_t_off[RS2_seg] ) * tdc_time; // RHRS  L-PMT(TOP-PMT)
  double time_Rb = (RTDC_F1FirstHit[RS2_seg+48] - RS2_b_off[RS2_seg] ) * tdc_time; // RHRS  R-PMT(BOTTOM)
  

  double time_Lt = (LTDC_F1FirstHit[LS2_seg]    - LS2_t_off[LS2_seg]) * tdc_time; // LHRS  L-PMT (TOP)
  double time_Lb = (LTDC_F1FirstHit[LS2_seg+48] - LS2_b_off[LS2_seg]) * tdc_time; // LHRS  R-PMT (BOT)
  // RHRS S2 TDC //
  ...
  double RS2_off[nS2],LS2_off[nS2],RS2_t_off[nS2],RS2_b_off[nS2],LS2_t_off[nS2],LS2_b_off[nS2];
 ...
 meantime_R= meantime_R - s2_tzero_R[RS2_seg] -s2_tzero_L[LS2_seg];
  ...
  //----- T0 correction -----//
  //  meantime_R +=  -s2_tzero_R[RS2_seg] - s2_tzero_L[LS2_seg];
...
void ana::MixedEvent(int rt, int lt){
  ...
  for(int i=0;i<nmixed;i++){
    R_tr_p_mix[i] = R_tr_p_mix[i+1];
    R_tr_px_mix[i] = R_tr_px_mix[i+1];
    R_tr_py_mix[i] = R_tr_py_mix[i+1];
  ...
  for(int i=0;i<nmixed;i++){
   ...
   R_E[i]     = sqrt( MK*MK + R_tr_p_mix[i]*R_tr_p_mix[i] );
    ...
    R_v_mix[i].SetXYZ(R_tr_px_mix[i], R_tr_py_mix[i], R_tr_pz_mix[i]);
    R_v_mix[i].RotateX(  13.2/180.*PI );
    L_v_mix[i].RotateX( -13.2/180.*PI );
...
void ana::MixedEventAll(int rt, int lt){
  ...
  for(int i=0;i<nmixed;i++){
    R_tr_p_mix_all[i]  = R_tr_p_mix_all[i+1];
    R_tr_px_mix_all[i] = R_tr_px_mix_all[i+1];
    R_tr_py_mix_all[i] = R_tr_py_mix_all[i+1];
    R_tr_pz_mix_all[i] = R_tr_pz_mix_all[i+1];
    R_tr_th_mix_all[i] = R_tr_th_mix_all[i+1];
    R_tr_ph_mix_all[i] = R_tr_ph_mix_all[i+1];
    
  }
  
  R_tr_p_mix_all[nmixed-1]   = R_p;
  R_tr_px_mix_all[nmixed-1]  = R_px;
  R_tr_py_mix_all[nmixed-1]  = R_py;
  R_tr_pz_mix_all[nmixed-1]  = R_pz;
  
  R_tr_th_mix_all[nmixed-1] = R_tr_tg_th[rt];
  R_tr_ph_mix_all[nmixed-1] = R_tr_tg_ph[rt];
  ...
  TVector3 L_v_mix_all[nmixed], R_v_mix_all[nmixed], B_v_mix_all[nmixed]; 
  
  for(int i=0;i<nmixed;i++){

   L_E[i]     = sqrt( Me*Me + L_p*L_p );
   R_E[i]     = sqrt( MK*MK + R_tr_p_mix_all[i]*R_tr_p_mix_all[i] );


    B_v_mix_all[i].SetXYZ(0.0,0.0,B_p);
    L_v_mix_all[i].SetXYZ(L_px, L_py, L_pz);
    R_v_mix_all[i].SetXYZ(R_tr_px_mix_all[i], R_tr_py_mix_all[i], R_tr_pz_mix_all[i]);
    R_v_mix_all[i].RotateX(  13.2/180.*PI );
    L_v_mix_all[i].RotateX( -13.2/180.*PI );

    mass_mix_all[i]=-10000.0;
    mass_mix_all[i] = sqrt( (Ee + mt - L_E[i] - R_E[i])*(Ee + mt - L_E[i] - R_E[i])
			- (B_v_mix_all[i] - L_v_mix_all[i] - R_v_mix_all[i])*(B_v_mix_all[i] - L_v_mix_all[i] - R_v_mix_all[i]) );

    //    cout<<" mass_mix_all "<<mass_mix_all[i]<<" mh "<<mh<<" i "<<i<<endl;
    mass_mix_all[i]=mass_mix_all[i] - mh;
    mass_mix_all[i]=mass_mix_all[i]*1000.; // GeV -> MeV  

    tr.Rp_mix_all[i]=R_tr_p_mix_all[i];
    tr.R_tr_th_mix_all[i]=R_tr_th_mix_all[i];
    tr.R_tr_ph_mix_all[i]=R_tr_ph_mix_all[i];

    //    cout<<"mm_mixed_all "<<mm_mixed_all[i]<<endl;
    
    if(fabs(mass_mix_all[i])>10000.)tr.mm_mixed_all[i] = -10000.;
    else tr.mm_mixed_all[i] = mass_mix_all[i];
...
/////////////////////
    ...
    for(int SEG=0;SEG<16;SEG++){
      tr.RS2T_F1[SEG]=-100000.;
      tr.RS2B_F1[SEG]=-100000.;
      tr.LS2T_F1[SEG]=-100000.;
      tr.LS2B_F1[SEG]=-100000.;
      tr.RS2T_F1_c[SEG]=-100000.;
      tr.RS2B_F1_c[SEG]=-100000.;
      tr.LS2T_F1_c[SEG]=-100000.;
      tr.LS2B_F1_c[SEG]=-100000.;
      tr.RS2T_F1_b[SEG]=-100000.;
      tr.RS2B_F1_b[SEG]=-100000.;
      tr.LS2T_F1_b[SEG]=-100000.;
      tr.LS2B_F1_b[SEG]=-100000.;         
      tr.Rtof[SEG]=-100000;
      tr.Ltof[SEG]=-100000.;
    }
    ...
    // ---- Tree Branch RHRS -------//
	  ...
	  if( L_Tr && L_FP && R_Tr && R_FP ){
	    ...
	    tr.zR_b   = R_tr_vz[rt];
	    tr.RXpt_b = R_tr_tg_th[rt];
	    tr.RYpt_b = R_tr_tg_ph[rt];
	    tr.momR_b = R_tr_p[rt];
	    tr.zL_b   = L_tr_vz[lt];
	    tr.LXpt_b = L_tr_tg_th[lt];
	    tr.LYpt_b = L_tr_tg_ph[lt];	    
	    tr.momL_b = L_tr_p[lt];
	    //-----------------------------//	    
	    ...
	    tr.dpk = Eloss(R_tr_tg_ph[rt],R_tr_vz[rt],"R");
	    tr.dpe_= Eloss(L_tr_tg_ph[lt],L_tr_vz[lt],"L");
	    
	    R_pb = R_tr_p[rt] + tr.dpk;
	    L_pb = L_tr_p[lt] + tr.dpe_;
	    B_pb = HALLA_p/1000.0 - tr.dpe;

	    //===================================//
	    B_p =-1000.;
	    B_p = HALLA_p/1000.0 - tr.dpe;
	    ...
	    tr.zR =R_tr_vz[rt];
	    tr.RXpt=R_tr_tg_th[rt];
	    tr.RYpt=R_tr_tg_ph[rt];
	    tr.momR = R_p;
	    tr.zL =L_tr_vz[lt];
	    tr.LXpt=L_tr_tg_th[lt];
	    tr.LYpt=L_tr_tg_ph[lt];	    
	    tr.momL = L_p;
...
void ana::Loop_c(){
  ...
  if(LHRS && RHRS && R_evtype==5){
    ...
    for(int lt=0;lt<NLtr;lt++){	
	  ...
	  tr.missing_mass_acc =-100000.;
	  tr.missing_mass_acc_all =-100000.;
	  tr.missing_mass_L   =-100000.;
	  tr.missing_mass_nnL =-100000.;
	  tr.missing_mass_H3L =-100000.;
	  tr.missing_mass_cut =-100000.;
	  ...
	  //	  if( L_Tr && L_FP && R_Tr && R_FP && padcut){
	    ...
	    //======== w/o Matrix Tuning ============//
	    double B_pb  = HALLA_p/1000.0 + Eloss(0.0,0,"B");
	    double L_pb  = L_tr_p[lt] - Eloss(L_tr_tg_ph[lt],L_tr_vz[lt],"L");
	    double R_pb  = R_tr_p[rt] - Eloss(R_tr_tg_ph[rt],R_tr_vz[rt],"R");
 	    double Ee_b = sqrt( Me*Me + B_pb*B_pb );
	    double L_Eb = sqrt( Me*Me + L_pb*L_pb );
	    double R_Eb = sqrt( MK*MK + R_pb*R_pb );
	   ...
	   // Kaon && ((-35<ct && ct<-15.0) || (15.0<ct && ct<35)) && zcut
	   if(((-35<ct && ct<-15.0) || (15.0<ct && ct<35)))MixedEventAll(rt,lt);
	   if(((-35<ct && ct<-15.0) || (15.0<ct && ct<35)) && Kaon && zcut)MixedEvent(rt,lt);	   


 if( Kaon && fabs(ct)<0.8  && track_flag && pathl_flag && zcut){
   h_mm_nnL_c  ->Fill( mm_nnL );
 }
...
void ana::MakeHist(){
  ...
  tree_out ->Branch("Rp_b"        ,&tr.momR_b      ,"momR_b/D"     );
  ...
  tree_out ->Branch("Rth_b"          ,&tr.RXpt_b        ,"RXpt_b/D"       );  
  tree_out ->Branch("Lth_b"          ,&tr.LXpt_b        ,"LXpt_b/D"       );  
  tree_out ->Branch("Rph_b"          ,&tr.RYpt_b        ,"RYpt_b/D"       );  
  tree_out ->Branch("Lph_b"          ,&tr.LYpt_b        ,"LYpt_b/D"       );  
  ...
  tree_out ->Branch("Rz_b"          ,&tr.zR_b        ,"zR_b/D"       );
  tree_out ->Branch("Lz_b"          ,&tr.zL_b        ,"zL_b/D"       );  
...
void ana::ReadParam(string name){
  ...
  RS2_t_off[i] = param-> GetTdcOffset(1, i, 1, 0); // RS2 TOP Offset
  RS2_b_off[i] = param-> GetTdcOffset(1, i, 1, 1); // RS2 BOT Offset
  LS2_t_off[i] = param-> GetTdcOffset(1, i, 1, 0); // LS2 TOP Offset
  LS2_b_off[i] = param-> GetTdcOffset(1, i, 1, 1); // LS2 BOT Offset  
