...
int main(int argc, char** argv){
  ...
  TFile* f1 = new TFile("/data3/root_ole/root2/tritium_111200.root"); // For the second H2 data (Nov10-, 2018)
  ...
  for(int i=0 ; i<ent ; i++){
	...
	if(timeL>0 && timeR>0 &&
	  ...
	  LenL  = LenL*PLr+PLm;
	  LenL  = pathl[0]; // by itabashi
	  //	  cout << LenL << endl;
	  //	  cout<<"LpathL "<<pathl[0]<<" Lpathlc "<<LenL<<endl;
	  ...
	  LenR  = LenR*PLr+PLm;
	  //	  cout<<"LpathR "<<pathl_R[0]<<" LpathRc "<<LenR<<endl;
	  LenR  = pathl_R[0]; // by itabashi
	  ...
	  cout <<"LenL "<< LenL << " pathl_L " << pathl[0] <<" corL "<<cor<<" betaL "<<beta << endl;
...
//	  cout << cor << " " << cor_R << endl;
	  cout <<"LenR "<< LenR << " pathl_R " << pathl_R[0] <<" corR "<<cor_R<<" betaR"<<beta_R << endl;
	  cout<<endl;
	  //	  cout << " LenR " << LenR << endl;
