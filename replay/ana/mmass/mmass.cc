...
int main(int argc, char** argv){
  ...
  string pngname;
  extern char *optarg;
  while((ch=getopt(argc,argv,"h:f:w:n:r:bcop:GHTtPU12"))!=-1){
    switch(ch){
    case 'f':
      ifname = optarg;
 ...
 //TChain //
  ...
  TChain* T;
  if(mode=="G"||mode=="T" ){T=new TChain("tree"); cout<<"tree branch : tree"<<endl;}
  else {T=new TChain("T");}
...
//==== AC Threshold variable (ACTH)===//
 ...
 TH1F* hmm_al_bg=new TH1F("hmm_al_bg","",bin_mm,min_mm,max_mm);
 set->SetTH1(hmm_al_bg,"MMass Al B.G","mmass [GeV]","Counts");
 TH1F* hmm_al_bg_woAC=new TH1F("hmm_al_bg_woAC","",bin_mm,min_mm,max_mm);
 set->SetTH1(hmm_al_bg_woAC,"MMass w/o AC cut Al B.G","mmass [GeV]","Counts");

 TH2F* hmm_al=new TH2F("hmm_al","",bin_mm,min_mm,max_mm,bin_al,min_al,max_al);
 set->SetTH2(hmm_al,"Missing mass w/ tuning AC cut vs Z ","mmass [GeV]","z [m]");
 TH2F* hmm_al_woAC=new TH2F("hmm_al_woAC","",bin_mm,min_mm,max_mm,bin_al,min_al,max_al);
 set->SetTH2(hmm_al_woAC,"Missing mass w/o AC cit vs Z ","mmass [GeV]","z [m]");
...
&& Ra1sum<th1_max){
 ...
 //--------- Missing mass Al Background study hist ----------//
 if(mode=="T" && coin_trig && cut_lpathl && cut_rpathl && cut_track && coin_tc<1.0 && -1.0<coin_tc && Ra1sum<0.1 && Ra2sum>2.0){
   hmm_al->Fill(mm,Rz);}// w/ tuning AC cut
 if(mode=="T" && coin_trig && cut_lpathl && cut_rpathl && cut_track && coin_tc<1.0 && -1.0<coin_tc){
<<<<<<< HEAD
   hmm_al->Fill(mm,Rz);}


=======
   hmm_al_woAC->Fill(mm,Rz);}//w/o AC cut


 //--------- Al Background hist ---------------//

 if(mode=="T" && (Rz>0.1 || Rz<0.1) && (Lz>0.1|| Lz<-0.1) && coin_trig && cut_lpathl && cut_rpathl && cut_track && coin_tc<1.0 && -1.0<coin_tc && Ra1sum<0.1 && Ra2sum>2.0)hmm_al_bg->Fill(mm);//w/ AC cut

 if(mode=="T" && (Rz>0.1 || Rz<0.1) && (Lz>0.1|| Lz<-0.1)&& coin_trig && cut_lpathl && cut_rpathl && cut_track && coin_tc<1.0 && -1.0<coin_tc)
   hmm_al_bg_woAC->Fill(mm);//w/o AC cut
>>>>>>> 5b5e523774a3ee3ea3692b6637ff1bb3bafae859
 ...
 //======= Al Background Fitting ==========//
 TF1* fgaus=new TF1("fgaus","gausn(0)",min_mm,max_mm);
 hmm_al_bg_woAC->Fit("fgaus","","Rq",min_mm,1.18);
 TF1* fal=new TF1("fal","gausn(0)+gausn(3)",min_mm,max_mm);
 double p0,p1,p2,p3,p4,p5;
 p0=fgaus->GetParameter(0);
 fal->SetParameter(0,p0);
 p1=fgaus->GetParameter(1);
 fal->SetParameter(1,p1);
 p2=fgaus->GetParameter(2);
 fal->SetParameter(2,p2);

 hmm_al_bg_woAC->Fit("fgaus","","Rq",1.18,max_mm);
 p3=fgaus->GetParameter(0);
 fal->SetParameter(3,p3);
 p4=fgaus->GetParameter(1);
 fal->SetParameter(4,p4);
 p5=fgaus->GetParameter(2);
 fal->SetParameter(5,p5);
 hmm_al_bg_woAC->Fit("fal","","Rq",min_mm,max_mm);
 hmm_al_bg_woAC->Fit("fal","","Rq",1.04,1.27);
 p0=fal->GetParameter(0);
 p1=fal->GetParameter(1);
 p2=fal->GetParameter(2);
 p3=fal->GetParameter(3);
 p4=fal->GetParameter(4);
 p5=fal->GetParameter(5);
 ...
 TF1* fL_al_peak[iter_max];
 TF1* fL_al_woAC_peak[iter_max];
 ...
 TH1D* hmm_al_p[iter_max];
 TH1D* hmm_al_woAC_p[iter_max];

 TGraphErrors* gL_al=new TGraphErrors();
 set->SetGr(gL_al,"Lambda S/N w/ AC cut Al dependence","z cut [m]","Lambda S/N ratio");
 TGraphErrors* gL_al_woAC=new TGraphErrors();
 set->SetGr(gL_al_woAC,"Lambda S/N w/o AC cut Al dependence","z cut [m]","Lambda S/N ratio");

 TGraphErrors* gLam_al=new TGraphErrors();
 set->SetGr(gLam_al,"Num of Lambda Al dependence","z cut [m]","N Lambda");
TGraphErrors* gLam_al_woAC=new TGraphErrors();
 set->SetGr(gLam_al_woAC,"Num of Lambda w/p AC cut Al dependence","z cut [m]","N Lambda");
 ...
 for(int k=0;k<nth;k++){
   hmm_al_p[k]=new TH1D(Form("hmm_al_p[%d]",k),"",bin_mm,min_mm,max_mm);
   set->SetTH1(hmm_al_p[k],"Missing mass Al dependence","mmass [GeV/c^2]","Coutns");
...
//---- Defolt parameters -----------//
 ...
 double L0_al[iter_max],L1_al[iter_max],L2_al[iter_max];
 double L0_al_woAC[iter_max],L1_al_woAC[iter_max],L2_al_woAC[iter_max];
 double sum_k_max=1250.;
 double fom_max=0.0;
 int bin_al_min,bin_al_max;
 double th_al,L_SN_al[iter_max],LBG_al[iter_max];

  int bin_al_woAC_min,bin_al_woAC_max;
 double LBG_al_woAC[iter_max],L_SN_al_woAC[iter_max];
...
hcoin_ac2_p[i][th1]->Add(hcoin_ac2_acc_p[i][th1],-1.0);
 ...
 if(th1==2 && th2==2){
   ...
   th_al=max_al-max_al/iter_ac1*i;
 bin_al_min=hmm_al->GetYaxis()->FindBin(min_al-min_al/iter_ac1*i);
 bin_al_max=hmm_al->GetYaxis()->FindBin(max_al-max_al/iter_ac1*i);

 hmm_al_p[i]=hmm_al->ProjectionX(Form("hmm_al_p[%d]",i),bin_al_min,bin_al_max);
 set->SetTH1(hmm_al_p[i],"Missing mass Al dependence","mmass [GeV/c^2]","Counts/2 MeV");

 //------ Lambda peak fit (gausn) ----------//
 fL_al_peak[i]=new TF1(Form("fL_al_peak[%d]",i),"gausn(0)",min_mm,max_mm);
 fL_al_peak[i]->SetParameter(0,2.15728e+01);
 fL_al_peak[i]->SetParameter(1,1.1156);
 fL_al_peak[i]->SetParameter(2,4.60534e-03);

 //fL_al[i]->SetParameter(3,1002.76);
 // fL_al[i]->SetParameter(4,937.229);
 hmm_al_p[i]->Fit(Form("fL_al_peak[%d]",i),"","Rq",1.1,1.13);
 L0_al[i]=fL_al_peak[i]->GetParameter(0);
 L1_al[i]=fL_al_peak[i]->GetParameter(1);
 L2_al[i]=fL_al_peak[i]->GetParameter(2);

 //------ Lambda peak fit (gausn+Al B.G. ) ----------//



 // fL_al[i]=new TF1(Form("fL_al[%d]",i),"gausn(0)+pol1(3)",min_mm,max_mm);
 fL_al[i]=new TF1(Form("fL_al[%d]",i),"gausn(0)+[9]*(gausn(3)+gausn(6))",min_mm,max_mm);

 fL_al[i]->SetParameter(0,L0_al[i]);
 fL_al[i]->SetParameter(1,L1_al[i]);
 fL_al[i]->SetParLimits(1,L1_al[i]-0.1*L2_al[i],L1_al[i]+0.1*L2_al[i]);
 fL_al[i]->SetParameter(2,L2_al[i]);
 fL_al[i]->FixParameter(3,p0);
 fL_al[i]->FixParameter(4,p1);
 fL_al[i]->FixParameter(5,p2);
 fL_al[i]->FixParameter(6,p3);
 fL_al[i]->FixParameter(7,p4);
 fL_al[i]->FixParameter(8,p5);

 //fL_al[i]->SetParameter(3,1002.76);
 // fL_al[i]->SetParameter(4,937.229);

 // hmm_al_p[i]->Fit(Form("fL_al[%d]",i),"","Rq",1.105,1.125);
 hmm_al_p[i]->Fit(Form("fL_al[%d]",i),"","Rq",1.05,1.28);
 L0_al[i]=fL_al[i]->GetParameter(0);
 L1_al[i]=fL_al[i]->GetParameter(1);
 L2_al[i]=fL_al[i]->GetParameter(2);

 L0_al[i]=L0_al[i]/0.002;// 2 MeV bin


 LBG_al[i]=hmm_al_p[i]->Integral(hmm_al_p[i]->FindBin(-3*L2_al[i]+L1_al[i]),hmm_al_p[i]->FindBin(+3*L2_al[i]+L1_al[i]));
 L_SN_al[i]=L0_al[i]/(LBG_al[i]-L0_al[i]);

 if(1000>L0_al[i] && L0_al[i]>0){ gLam_al->SetPoint(i,th_al,L0_al[i]);
 if(L_SN_al[i]>0)gL_al->SetPoint(i,th_al,L_SN_al[i]);

}

 
 //====== w/o AC cut Missing mass hist =========//

 bin_al_woAC_min=hmm_al_woAC->GetYaxis()->FindBin(min_al-min_al/iter_ac1*i);
 bin_al_woAC_max=hmm_al_woAC->GetYaxis()->FindBin(max_al-max_al/iter_ac1*i);

 hmm_al_woAC_p[i]=hmm_al_woAC->ProjectionX(Form("hmm_al_woAC_p[%d]",i),bin_al_min,bin_al_max);
 set->SetTH1(hmm_al_woAC_p[i],"Missing mass Al dependence w/o AC cut","mmass [GeV/c^2]","Counts/2 MeV");

 //------ Lambda peak fit (gausn) ----------//
 fL_al_woAC_peak[i]=new TF1(Form("fL_al_woAC_peak[%d]",i),"gausn(0)",min_mm,max_mm);
 fL_al_woAC_peak[i]->SetParameter(0,4.4095);
 fL_al_woAC_peak[i]->SetParameter(1,1.1158);
 fL_al_woAC_peak[i]->SetParameter(2,5.210534e-03);
 //fL_al_woAC[i]->SetParameter(3,1002.76);
 // fL_al_woAC[i]->SetParameter(4,937.229);
 hmm_al_woAC_p[i]->Fit(Form("fL_al_woAC_peak[%d]",i),"","Rq",1.11,1.125);
 L0_al_woAC[i]=fL_al_woAC_peak[i]->GetParameter(0);
 L1_al_woAC[i]=fL_al_woAC_peak[i]->GetParameter(1);
 L2_al_woAC[i]=fL_al_woAC_peak[i]->GetParameter(2);
 //------ Lambda peak fit (gausn+pol1) ----------//

 // fL_al_woAC[i]=new TF1(Form("fL_al_woAC[%d]",i),"gausn(0)+pol1(3)",min_mm,max_mm);
 fL_al_woAC[i]=new TF1(Form("fL_al_woAC[%d]",i),"gausn(0)+[9]*(gausn(3)+gausn(6))",min_mm,max_mm);
 fL_al_woAC[i]->SetParameter(0,L0_al_woAC[i]);
 fL_al_woAC[i]->SetParameter(1,L1_al_woAC[i]);
 fL_al_woAC[i]->SetParLimits(1,L1_al_woAC[i]-0.1*L2_al_woAC[i],L1_al_woAC[i]+0.1*L2_al_woAC[i]);
 fL_al_woAC[i]->SetParameter(2,L2_al_woAC[i]);
 fL_al_woAC[i]->FixParameter(3,p0);
 fL_al_woAC[i]->FixParameter(4,p1);
 fL_al_woAC[i]->FixParameter(5,p2);
 fL_al_woAC[i]->FixParameter(6,p3);
 fL_al_woAC[i]->FixParameter(7,p4);
 fL_al_woAC[i]->FixParameter(8,p5);

 //fL_al_woAC[i]->SetParameter(3,1002.76);
 // fL_al_woAC[i]->SetParameter(4,937.229);


 // hmm_al_woAC_p[i]->Fit(Form("fL_al_woAC[%d]",i),"","Rq",1.105,1.125);
 hmm_al_woAC_p[i]->Fit(Form("fL_al_woAC[%d]",i),"","Rq",1.05,1.28);
 L0_al_woAC[i]=fL_al_woAC[i]->GetParameter(0);
 L1_al_woAC[i]=fL_al_woAC[i]->GetParameter(1);
 L2_al_woAC[i]=fL_al_woAC[i]->GetParameter(2);

 L0_al_woAC[i]=L0_al_woAC[i]/0.002;// 2 MeV bin

 LBG_al_woAC[i]=hmm_al_woAC_p[i]->Integral(hmm_al_woAC_p[i]->FindBin(-3*L2_al_woAC[i]+L1_al_woAC[i]),hmm_al_woAC_p[i]->FindBin(+3*L2_al_woAC[i]+L1_al_woAC[i]));
 L_SN_al_woAC[i]=L0_al_woAC[i]/(LBG_al_woAC[i]-L0_al_woAC[i]);
 if(L0_al_woAC[i]>0.0 && L0_al_woAC[i]<1200){
   gLam_al_woAC->SetPoint(i,th_al,L0_al_woAC[i]);
 if(L_SN_al_woAC[i]>0)gL_al_woAC->SetPoint(i,th_al,L_SN_al_woAC[i]);
 }
...
TLine* lac=new TLine(min_coin_c,ac1_adc[0],max_coin_c,ac1_adc[0]);
 ...
 hmm_ac2[2]->Draw("colz");
  ...
  gLam_al->Draw("AP");
  c26->cd(4);
  gLam_al_woAC->Draw("AP");
  TCanvas* c27=new TCanvas("c27","c27");
  c27->Divide(3,4);
  TCanvas* c28=new TCanvas("c28","c28");
  c28->Divide(3,4);
  for(int i=0;i<12;i++){
    if(iter_ac1<48)break;
    int l=4*i;
    c27->cd(i+1);
      hmm_al_p[l]->Draw();
      fL_al[l]->SetLineColor(2);
      fL_al[l]->Draw("same");
      c28->cd(i+1);
      hmm_al_woAC_p[l]->Draw();
      fL_al_woAC[l]->SetLineColor(2);
      fL_al_woAC[l]->Draw("same");
...
c12->Print(name+"]","pdf");
 ...
 if(root_flag){
   ...
   hmm_al_woAC->Write();
   hmm_al_bg->Write();
   hmm_al_bg_woAC->Write();
   //   hmm_al_p->Write();
   for(int i=0;i<iter_max;i++){hmm_al_p[i]->Write();hmm_al_woAC_p[i]->Write();}
   gL_al->SetName("gL_al");
   gL_al_woAC->SetName("gL_al_woAC");
   gLam_al->SetName("gLam_al");
   gLam_al_woAC->SetName("gLam_al_woAC");
			 gL_al->Write();
			 gL_al_woAC->Write();
			 gLam_al->Write();
		         gLam_al_woAC->Write();
