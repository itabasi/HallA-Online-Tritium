...
void vdct0_Left(int run) {
  //   gROOT->SetBatch(1);      
   ...
   // const TString ROOTfilePath = "/chafs1/work1/tritium/rootfiles_1.6_root6";
    ...
    TString plotname = Form("../pdf/VDC/L_vdct0_%d.pdf",run);
    GmpVDCt0Calib* vdct0 = new GmpVDCt0Calib;
    //    vdct0->Calibrate(T,1,"(DL.evtypebits>>2)&1",plotname);
        vdct0->Calibrate(T,1,"(DR.evtypebits==2)",plotname);
   //   vdct0->Calibrate(T,1,"DL.evtypebits==4",plotname);
    vdct0->SaveDatabase(Form("./DB/db_L.vdc.%d.dat",run));
    ...
    c1->SaveAs(Form("../pdf/VDC/Lvdct0plot_%d.pdf",run));
    //    gSystem->Exit(1);
