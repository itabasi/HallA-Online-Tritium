...
void vdct0_Right(int run) {
    gROOT->SetBatch(1);  
    gSystem->Load("GmpVDCt0Calib_cc.so");
    ...
    TString plotname = Form("../pdf/VDC/R_T5vdct0_%d.pdf",run);
    GmpVDCt0Calib* vdct0 = new GmpVDCt0Calib;
    //    vdct0->Calibrate(T,0,"(DR.evtypebits>>5&1)",plotname);
    vdct0->Calibrate(T,0,"(DR.evtypebits==32)",plotname);
    //Trigger==4 (RHRS S0 &S2)
    vdct0->SaveDatabase(Form("./DB/db_R_T5_.vdc.%d.dat",run));
    ...
    for (UInt_t ii=0; ii<GmpVDCt0Calib::GetNumberOfPlanes(); ii++) {
	  ...
	  Double_t offset = vdct0->GetOffset(ii,jj); // taken by fitting 
    ...
    c1->SaveAs(Form("../pdf/VDC/Rvdct0_T5plot_%d.pdf",run));
