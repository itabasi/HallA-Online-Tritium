    ...
    if(LHRS && RHRS && R_evtype==5){
	  ...
	  if( R_a1_asum_c<150 && R_a2_asum_c>1000 && R_a2_asum_c<6000) Kaon = true;
	    ...
	    //========== Before Tuned Parameters =========//
            ...
            double mass, mm,mass_L,mass_nnL,mm_L,mm_nnL,mm_Al,mass_Al,mass2,mm2,mt;
	   ...
	   mm_L=mass_L;
		...
		//                h_mm      ->Fill( mm );
	     ...
	     if( fabs( L_tr_vz[lt] - 0.01 ) < 0.1 && fabs( R_tr_vz[rt] - 0.01 ) < 0.1 &&fabs(ct)<0.9)h_mm->Fill( mm ); //No Kaon Cut
	     if( fabs( L_tr_vz[lt] - 0.01 ) < 0.1 && fabs( R_tr_vz[rt] - 0.01 ) < 0.1 && 2.0<ct && ct<4.0)h_mm_pi->Fill( mm ); //No Kaon Cut
	     if( fabs( L_tr_vz[lt] - 0.01 ) < 0.1 && fabs( R_tr_vz[rt] - 0.01 ) < 0.1 
...
// COIN
 ...
 TCanvas *c14 = new TCanvas("c14","Missing Mass 1",1000,800);
   ...
   // h_mmbg->Scale(1.8/160.);  h_mmbg ->Draw("same");
    ...
    c14->cd(6)->SetMargin(0.15,0.15,0.15,0.10); gPad->SetLogy(0); h_mm_Al       ->Draw();
                      h_mm_acc->Scale(1.8/40.); gPad->SetLogy(0); h_mm_Al_acc   ->Draw("same");
                                                                  h_peak_Al     ->Draw("same");
    ...
    c14->cd(9)->SetMargin(0.15,0.15,0.15,0.10); gPad->SetLogy(0); h_peak_mm     ->Draw();
                                           h_acc_nnL->Scale(1.8/40.);  h_mm_pi->Draw("same"); 
...
/////////////////////
  ...
  max_Lp=2.4;
  #endif
  int bin_mm=(int)(max_mm-min_mm)*500; // 2MeV/bin;
  int bin_Lp=200;


  
  h_mm       = new TH1D("h_mm"      ,"h_mm"      , bin_mm,min_mm,max_mm);  //Lambda mass range bin=2 MeV
  h_mm_acc   = new TH1D("h_mm_acc"  ,"h_mm_acc"  , bin_mm,min_mm,max_mm);  //Lambda mass range bin=2 MeV
  h_peak_mm       = new TH1D("h_peak_mm"      ,"h_peak_mm"      , bin_mm,min_mm,max_mm); //Lambda mass range bin=2 MeV
