	...
	if(fcent[j]-selection_width<Zt[0]
	  ...
	  if(offset_flag[j]==true || offset_flag[j]==false){
		...
		if(pow(ssx-(refx[nhole]-0.8),2.0)/pow(selec_widthx,2.0)
		   + pow(ssy-(refy[nhole]-0.3),2.0)/pow(selec_widthy,2.0)<0.3){
		  holeg_temp = nhole;
		  foilg_temp = j;
  ...
  for (int i=0 ; i< ent ; i++){
   ...
   //      else t2->GetEntry(i-ent+evshift);
    ...
    hth->Fill(Xpt[0]);
      ...
      if(fabs(Xpt[0])<100)cout<<"i "<<i<<" Xpt "<<Xpt[0]<<" Ypt "<<Ypt[0]<<" z "<<Zt[0]<<endl;
    ...
    //    if(fabs(Xpt[0])<100)cout<<"Xpt "<<Xpt_tuned[0]<<" Ypt "<<Ypt_tuned[0]<<" z "<<Zt[0]<<endl;
...
// #############################################################
{
  ...
  //  for(int i=0 ; i<nfoil ; i++){
	...
	chi2[i][j] = chi2[i][j]/nev[i][j]/pow(sigma,2.0);
      ...
      cout<<"chi2 "<<chi2[i][j]<<" total chi2 "<<total_chi2<<endl;
    }
    //  }
