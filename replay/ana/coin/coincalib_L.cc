...
void coincalib::CoinCalc(int RS2_seg, int LS2_seg, int rhit, int lhit){
  ...
  //  if(mode<=0)
    Rtof_c = RS2_F1time[RS2_seg] - R_pathl_c/(Beta_R*LightVelocity);
    //  if(mode>=0)
    Ltof_c = LS2_F1time[LS2_seg] - L_pathl_c/(Beta_L*LightVelocity);  
...
double coincalib::tune(double* pa, int MODE) 
{
  ...
  for(int i=0 ; i<allparam ; i++)
    minuit -> GetParameter(i,Pct_L[i],er);// LHRS momentum parameters
...
int main(int argc, char** argv){
  ...
  //  if(MODE>=0)
    Coin->MTParam_L(mtparam_L);
  //  if(MODE<=0)
    Coin->MTParam_R(mtparam_R);
