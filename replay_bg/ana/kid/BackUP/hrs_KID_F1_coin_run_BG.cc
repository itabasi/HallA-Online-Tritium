...
int main(int argc, char** argv){
 ...
 //=============================================================//
  ...
  string buf, runname[1000],run_num[1000],def_run,memo[1000];
  int run[1000],sub_run[1000];
  int l=0;
  while(1){
    getline(ifp,buf);
    if( buf[0]=='#' ){ continue; }
    ...
    sbuf >> runname[l];
    run_num[l]=runname[l].substr(42,6);
    sscanf(run_num[l].c_str(),"%d",run[l]);
    cout<<Form("num[%d]",l)<<run[l]<<endl;
    
    if(l>1 && run[l]!=run[l-1]){
    j=j+1; 
    sub_run[j]=0;
    }else {sub_run[j]=sub_run[j]+1;
      cout<<"sub_run: "<<sub_run[j]<<endl;
    }
    l=l+1;
    }
...
if(output_flag==1)gSystem->Exit(0);
