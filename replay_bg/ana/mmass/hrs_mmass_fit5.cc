...
void hrs_mmass_fit5(){
 ...
 TH1F* hmm=new TH1F("hmm","MM AC1 &AC2 no cut",bin_mm,min_mm,max_mm);
 ...
 cout<<"========= coincidence =============== "<<endl;
  ...
  //  bg[0]=-176.072;
  // bg[1]=159.562;
  fbg->SetParameter(0,bg[0]);
  fbg->SetParameter(1,bg[1]);
  ...
  fLam->SetParameter(3,bg[0]);
  fLam->SetParameter(4,bg[1]);
  ...
  fLam->SetParLimits(2,0.8*p_L[2],1.2*p_L[2]);
   hmm_ac_p->Fit("fLam","","",1.08,1.14);
  //  hmm_ac_p->Fit("fLam","","",1.0,1.1);
  ...
  fSig->FixParameter(3,bgS[0]);
  fSig->FixParameter(4,bgS[1]); 
  //fSig->FixParameter(5,bgS[2]);
 //  fSig->FixParameter(2,bg[2]);
  hmm_ac_p->Fit("fSig","","",1.15,1.25);
  ...
  double PL,PS;
  PL=hmm_ac_p->Integral(hmm_ac->FindBin(p_L[1]-3*p_L[2]),hmm_ac->FindBin(p_L[1]+3*p_L[2]));
  PS=hmm_ac_p->Integral(hmm_ac->FindBin(p_S[1]-3*p_S[2]),hmm_ac->FindBin(p_S[1]+3*p_S[2]));

  double BGL=fbg->Integral(p_L[1]-3*p_L[2],p_L[1]+3*p_L[2]);
  ...
  cout<<"Lambda Integral:"<<PL<<endl;
  cout<<"Lambda BG Integral:"<<BGL/0.002<<endl;
  cout<<"integral reign: "<<p_L[1]-3*p_L[2]<<"< mm <"<<p_L[1]+3*p_L[2]<<endl;
  cout<<"NL"<<NL<<endl;
 ...
 TLine* lac=new TLine(p_L[1]-3*p_L[2],0,p_L[1]-3*p_L[2],100.);
 lac->SetLineWidth(2);
 lac->SetLineColor(6);
  ...
  //hcoin_p->Draw();
  //  hmm_ac_p->Fit("fbg","","",1.08,1.14);
  //  hmm_ac_p->GetXaxis()->SetRangeUser(1.08,1.14);
  hmm_ac_p->Draw();
  fLam->SetLineColor(2);
  fLam->Draw("same");
  //lac->DrawLine(p_L[1]-3*p_L[2],0,p_L[1]-3*p_L[2],100.);
  //lac->DrawLine(p_L[1]+3*p_L[2],0,p_L[1]+3*p_L[2],100.);
//p_L[1]-3*p_L[2]
