...
// const double  Xptr=1., Yptr=1., Momr=1.;
 ...
 tree->SetBranchAddress("KXFP2",&XFP);
 tree->SetBranchAddress("KXpFP2",&XpFP);
 tree->SetBranchAddress("KYFP2",&YFP);
 tree->SetBranchAddress("KYpFP2",&YpFP);
 tree->SetBranchAddress("KXt",&Xt);
 tree->SetBranchAddress("KYt",&Yt);
 tree->SetBranchAddress("KZt",&Zt);
...
//     
	...
	//&& fabs(ztarL[0]-ztarR[0])<0.05
     ...
     if(goodflag==true){
       ...
       fVal[jj][0]=XFP->at(0);//[cm]
       fVal[jj][1]=XpFP->at(0);//[rad]
       fVal[jj][2]=YFP->at(0);//[cm]
       fVal[jj][3]=YpFP->at(0);//[rad]
