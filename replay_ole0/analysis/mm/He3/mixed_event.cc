...
void mixed_event(){
  TFile* f1 = new TFile("../He3_20191212.root");
  ...
  //double xmin = -300.0, xmax = 300.0; int xbin = 100; // 6 MeV / bin
  double xmin = -300.0, xmax = 300.0; int xbin = 60; // 10 MeV / bin
  ...
  //h2->GetYaxis()->SetTitle("Counts / 6 MeV");
  h2->GetYaxis()->SetTitle("Counts / 10 MeV");
  ...
  for(int i=0 ; i<ent ; i++){
	...
	for(int j=0 ; j<(int)nmix ; j++){
	  ...
	  mixed_mm = (mixed_mm - m_dL) * 1000.; // Gen --> MeV
